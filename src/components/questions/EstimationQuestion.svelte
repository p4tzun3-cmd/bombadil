...
<div class="flex flex-col items-center justify-center h-full gap-6">
	{#if cycleStep === 'PROMPT'}
		<div class="text-center px-12">
			<p class="text-5xl font-bold text-[var(--color-text)] leading-tight">{question.prompt}</p>
		</div>

	{:else if cycleStep === 'MEDIA' && question.media}
		<MediaPlayer media={question.media} />

	{:else if cycleStep === 'ANSWER'}
		<div class="text-center">
			<p class="text-2xl text-[var(--color-text)] opacity-60 mb-4">{question.prompt}</p>
